...
#define KTagCornerWidth    15.0f
#define KTagFont           14.0f
...
-(void)creatUI:(NSMutableArray *)tagArr{
    ...
    [self.tagArr enumerateObjectsUsingBlock:^(id value, NSUInteger idx, BOOL *stop) {
        ...
        Size_str.width += K_Tag_Title_H_Marin*3;
        Size_str.height += K_Tag_Title_V_Marin*3;
        ...
        if (idx==self.tagArr.count-1) {
            [self setHight:self andHight:tagView_height+Size_str.height + 10];
        }
        ...
        if (self.is_can_addTag&&idx==self.tagArr.count-1) {//能添加状态&&最后一个-->(进入添加tag界面不用显示删除图片)
            ...
            if (self.tagStateType==1) {
                ...
                //删除图片可以换成自己的图片
                removeImage.image = [UIImage imageNamed:@"search_nav_icon_delete_s"];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if ([self.delegate respondsToSelector:@selector(touchesEndedClick)]) {
        [self.delegate touchesEndedClick];
    }
}
