...
@implementation AccountLongCell

+ (instancetype)cellWithTableView:(UITableView *)tableView{
    
    NSString *ID = @"AccountLongCell";
    AccountLongCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) {
        cell = [[AccountLongCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
    return cell;
}
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        _titleLab.frame = CGRectMake(15, 12, 60, 30);
