...
- (id)initWithFrame:(CGRect)frame direction:(LightControlDirection)direction{
    ...
    if (self) {
        ...
        _slidedLineColor = ADColor(22, 122, 251);
...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat startLineX = (_direction == LightControlDirectionHorizonal ? _circleRadius : (self.frame.size.width - _lineWidth) / 2);
    CGFloat startLineY = (_direction == LightControlDirectionHorizonal ? (self.frame.size.height - _lineWidth) / 2 : _circleRadius);//起点
    
    CGFloat endLineX = (_direction == LightControlDirectionHorizonal ? self.frame.size.width - _circleRadius : (self.frame.size.width - _lineWidth) / 2);
    CGFloat endLineY = (_direction == LightControlDirectionHorizonal ? (self.frame.size.height - _lineWidth) / 2 : self.frame.size.height- _circleRadius);//终点
    ...
    CGFloat slidedLineX = (_direction == LightControlDirectionHorizonal ? MAX(_circleRadius, (_ratioNum * self.frame.size.width - _circleRadius)) : startLineX);
    
    CGFloat slidedLineY = (_direction == LightControlDirectionHorizonal ? startLineY : MAX(_circleRadius, (_ratioNum * self.frame.size.height - _circleRadius)));
    
    CGContextMoveToPoint(context, startLineX, startLineY);
    CGContextAddLineToPoint(context, slidedLineX - 3.5, slidedLineY);
    CGContextClosePath(context);
    CGContextStrokePath(context);
    ...
    //
    CGFloat circleX = (_direction == LightControlDirectionHorizonal ? MAX(_circleRadius + penWidth, slidedLineX - penWidth ) : startLineX);
    CGFloat circleY = (_direction == LightControlDirectionHorizonal ? startLineY : MAX(_circleRadius + penWidth, slidedLineY - penWidth));
...
- (void)updateTouchPoint:(NSSet*)touches {
    ...
    self.ratioNum = (_direction == LightControlDirectionHorizonal ? touchPoint.x : touchPoint.y) / (_direction == LightControlDirectionHorizonal ? self.frame.size.width : self.frame.size.height);
