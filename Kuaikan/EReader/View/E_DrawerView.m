...
//        backgroundView.image = background;
        backgroundView.backgroundColor = ADColorRGBA(30, 30, 30, 0.5);
...
- (void)configUI{
    ...
    if (_listView == nil) {
        _listView = [[E_ListView alloc] initWithFrame:CGRectMake(- ListViewW, 0, ListViewW, self.frame.size.height) andDataArray:self.dataArray];
        _listView.delegate = self;
        _listView.bookID = _bookID;
        _listView.index = _myIndex;
...
-(NSArray *)dataArray{
    if (_dataArray == nil || _dataArray.count<20) {
...
- (void)clickChapter:(NSInteger)chaperIndex{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        [_delegate turnToClickChapterLastPate];
        [self removeFromSuperview];
