...
//    [window insertSubview:self belowSubview:[AppDelegate sharedApplicationDelegate].nightView];
    ...
    _bottomView.frame = CGRectMake(0, ScreenHeight - 370, ScreenWidth, 370);
...
//    _remarkLabel.textColor = LightTextColor;
    ...
    _ensureBtn.frame = CGRectMake(130, 310, ScreenWidth - 130, 60);
...
//    [_ensureBtn setTitleColor:WhiteColor forState:UIControlStateNormal];
    ...
    allView.frame = CGRectMake(0, 310, 130, 60);
...
- (void)rechareButtonClick:(UIButton *)button{
    ...
    if ([self.delegate respondsToSelector:@selector(clickOneTwoOrThree:)]) {
        [self.delegate clickOneTwoOrThree:button];
    }
}
...
- (void)ensureClick:(EnsureButton *)button{
    ...
    if ([self.delegate respondsToSelector:@selector(threeOrderViewEnsureBtnClick:)]) {
        [self.delegate threeOrderViewEnsureBtnClick:button];
    }
}
...
- (void)closeBtnClick{
    ...
    //if ([self.delegate respondsToSelector:@selector(threeOrderViewCancelBtnClick)]) {
    //    [self.delegate threeOrderViewCancelBtnClick];
   // }
